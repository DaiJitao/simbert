...
savedir = 'model'
datadir = 'data'
filepath = "qtSim_cosin_nofrozen_middle_{epoch:02d}_{mae:.4f}.h5"
modelfile = os.path.join(savedir, filepath)
trainf, testf, validatef = datadir + '/test_data_5b.txt', datadir + '/test_data_5b.txt', datadir + '/test_data_5b.txt'
...
if __name__ == '__main__':
    ...
    emb = Embedding(input_dim=23000, output_dim=128, name='embedding')(concated_vec)

    qt_vec = Dense(units=512, name='qt-dense-1')(emb)
    qt_vec = BatchNormalization(name='norm-1')(qt_vec)
    qt_vec = Activation('relu')(qt_vec)

    qt_vec = Dense(units=512, name='qt-dense-2')(qt_vec)
    qt_vec = BatchNormalization(name='norm-2')(qt_vec)
    qt_vec = Activation('relu')(qt_vec)

    qt_vec = Dense(units=256, activation='relu', name='qt-dense-3')(qt_vec)
    qt_vec = BatchNormalization(name='norm-3')(qt_vec)
    qt_vec = Activation('relu')(qt_vec)

    qt_vec = Dense(units=128, activation='relu', name='qt-dense-4')(qt_vec) # (64,128)
    qt_vec = Reshape([-1])(qt_vec)
    ...
    query_output = Dense(units=768, activation='relu', kernel_initializer=bert.initializer, name='query-dense-1')(
        query_output)
    title_output = Dense(units=768, activation='relu', kernel_initializer=bert.initializer, name='title-dense-1')(
        title_output)
    query_output = Dropout(rate=0.5, name='query-drop-2')(query_output)
    title_output = Dropout(rate=0.5, name='title-drop-2')(title_output)
    query_output = Dense(units=512, activation='relu', kernel_initializer=bert.initializer, name='query-dense-2')(
        query_output)
    title_output = Dense(units=512, activation='relu', kernel_initializer=bert.initializer, name='title-dense-2')(
        title_output)
