#! -*- coding:utf-8 -*-
# qt sim cosine任务训练:middle;加三层relu,一个头；
# sentence bert代码，验证训练可用
...
from keras.layers import Dropout, Dense, Lambda, Concatenate, Reshape, GlobalAveragePooling1D, BatchNormalization, Activation
...
if isTestPlatform:
	...
	batch_size = 512  # 384 #256
	epochs = 2
	use_gpu = ''
	gpus = 0
	datapath = 'data/'
	valid_f = datapath + 'test_data_1bai.txt'
	train_f = datapath + 'test_data_1bai.txt'
	test_f = datapath + 'test_data_1bai.txt'
	print('-->train data:{}, test data:{}, validate data:{}'.format(train_f, test_f, valid_f))

	pretrained_path = 'models/chinese_simbert_L-6_H-384_A-12'
...
if __name__ == '__main__':
	train = True
	savedir='model'
	modelname="qtSim_sentence_bert_middle_{epoch:02d}_{mae:.4f}.h5"
	modelfile=os.path.join(savedir,modelname)
	print('-->head1 middle model epochs:' + str(epochs))
	...
	print("-->bert1 model output:", bert1.output)  # shape=(None, 384)
	print("-->bert2 model output:", bert2.output)  # shape=(None, 384)
