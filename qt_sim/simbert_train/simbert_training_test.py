...
class data_generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        batch_negative_token_ids, batch_negative_segment_ids = [], []
        for is_end, d in self.sample(random):
            ...
            token_ids, segment_ids = tokenizer.encode(text, synonym, max_length=maxlen * 2)
            batch_token_ids.append(token_ids)
            batch_segment_ids.append(segment_ids)
            label = np.random.randint(0, high=2)
            batch_labels.append([label])  # 真实标签
            # 加入负样本
            negative_text1 = '我是负样本，用于测试的'
            token_ids, segment_ids = tokenizer.encode(text, negative_text1, max_length=maxlen * 2)
            batch_negative_token_ids.append(token_ids)
            batch_negative_segment_ids.append(segment_ids)
            # 调换同义句子对位置
            token_ids, segment_ids = tokenizer.encode(synonym, text, max_length=maxlen * 2)  # 调换位置
            batch_token_ids.append(token_ids)
            batch_segment_ids.append(segment_ids)
            label = np.random.randint(0, high=2)
            batch_labels.append([label])  #真实标签
            # 加入负样本
            token_ids, segment_ids = tokenizer.encode(synonym, negative_text1, max_length=maxlen * 2)
            batch_negative_token_ids.append(token_ids)
            batch_negative_segment_ids.append(segment_ids)
            if len(batch_token_ids) == self.batch_size or is_end:
                ...
                batch_negtok_ids = sequence_padding(batch_negative_token_ids)
                batch_negseg_ids = sequence_padding(batch_negative_segment_ids)
                yield [batch_token_ids, batch_segment_ids, batch_labels, batch_negtok_ids, batch_negseg_ids], None
                batch_token_ids, batch_segment_ids, batch_labels = [], [], []
                batch_negative_token_ids, batch_negative_segment_ids = [], []
...
class TotalLoss(Loss):
    ...
    def compute_loss_of_seq2seq(self, inputs, mask=None):
        y_true, y_mask, label, negatived_token_ids, negative_segment_ids, _, y_pred = inputs
    ...
    def compute_loss_of_similarity(self, inputs, mask=None):
        _, _, label, negatived_token_ids, negative_segment_ids, y_pred, _ = inputs
...
class Evaluate(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if logs['loss'] <= self.lowest:
            ...
            # model.save_weights('qt_sim_models/latest_model.weights')
            mf = modelfile.format(epoch, '%.5f' % logs['loss'])
            # model.save_weights(mf) #model.save(mf)
            model.save(mf)
...
epochs = 5
padding_size = 128
train_data = 'data/validate_data.txt'
...
modelfile = 'data/TestqtSimbertModelGPU_Test_{}_{}.h5'  # 保存模型
...
if __name__ == '__main__1':
    ...
    res, label = next(gen)
    print(len(res))
    batch_token_ids, batch_segment_ids, batch_labels, batch_negtok_ids, batch_negseg_ids = res
    print('batch_token_ids ',len(batch_token_ids))
    print('batch_segment_ids ', len(batch_segment_ids))
    print('batch_labels ', len(batch_labels))
    print('batch_negtok_ids ', len(batch_negtok_ids))
    print('batch_negseg_ids ', len(batch_negseg_ids))
...
if __name__ == '__main__':
    ...
    with strategy.scope():
        ...
        inputs = bert.model.inputs
        label_input = Input(shape=(1,), name='label_input')
        token_input = Input(shape=(None, ), name='token_input')
        segment_input = Input(shape=(None, ), name='segment_input')
        inputs.append(label_input)
        inputs.append(token_input)
        inputs.append(segment_input)
        outputs = TotalLoss()(inputs + bert.model.outputs) # TotalLoss([2, 3])(inputs + bert.model.outputs)
        print('type:', type(outputs), dir(outputs))
