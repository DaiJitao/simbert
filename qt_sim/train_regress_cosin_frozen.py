...
savedir = 'model'
datadir = 'data'
filepath = "qtSimNewData_cosin_fronzenModel_{epoch:02d}_{mae:.4f}.h5"
...
if __name__ == '__main__':
	...
	with strategy.scope():
		...
		query_output = Dropout(rate=0.1, name='query-drop-1')(bert.model.output)
		title_output = Dropout(rate=0.1, name='title-drop-1')(bert.model.output)
		query_output = Dense(units=768, activation=tf.nn.tanh, kernel_initializer=bert.initializer, name='query-1')(query_output)
		title_output = Dense(units=768, activation=tf.nn.tanh, kernel_initializer=bert.initializer, name='title-1')(title_output)
		query_output = Dropout(rate=0.1, name='query-drop-2')(query_output)
		title_output = Dropout(rate=0.1, name='title-drop-2')(title_output)
		query_output = Dense(units=300, activation=tf.nn.tanh, kernel_initializer=bert.initializer, name='query-2')(query_output)
		title_output = Dense(units=300, activation=tf.nn.tanh, kernel_initializer=bert.initializer, name='title-2')(title_output)
		...
		# 最后一层作为输出 model = keras.models.Model(bert.model.input, bert.model.output)
