...
# bert配置
config_path = 'models/chinese_simbert_L-12_H-768_A-12/bert_config.json'  # models/chinese_simbert_L-12_H-768_A-12/bert_config.json
...
class Evaluate(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        mf = modelfile.format(epoch, '%.3f' %logs['loss'])
        model.save(modelfile.format(epoch, logs['loss']))
        # 保存最优
...
if __name__ == '__main__':
    modelfile = 'data/qtSim_model_{}epoch_{}.h5'
    ...
    batch_size = 10
    model.load_weights('qt_sim_models/latest_model.weights')
    print('-->加载模型成功！')
    train_generator = data_generator(read_corpus(), batch_size)
    gen_data = train_generator.forfit()
    data = next(gen_data)
    data = data[0]
    res = model.predict(data)
    print('预测',res[0].shape)
